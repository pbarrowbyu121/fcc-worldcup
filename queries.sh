...
echo "$($PSQL "SELECT name FROM (SELECT g.year, g.round, t.name FROM games AS g LEFT JOIN teams AS t ON g.winner_id=t.team_id UNION SELECT g.year, g.round, t.name FROM games AS g LEFT JOIN teams AS t ON g.opponent_id=t.team_id) as team_names WHERE year=2014 AND round='Eighth-Final'")"
